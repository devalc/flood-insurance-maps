...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 07 20:00:00 2025
@author: cdeval
"""
import duckdb
import pandas as pd
...
def plot_single_frame(ax_main, ax_insets, merged_conus, merged_nonconus, inset_defs, title_suffix, color_dict_no_data_key):
    """
    Plots a single frame of the map, clearing existing artists.
    `color_dict_no_data_key` is passed instead of `color_dict` to ensure the 'NoData' color is accessible.
    The title_suffix here will be just the event name.
    """
    ax_main.clear() # Clear main axes for each frame
    for ax_inset in ax_insets.values():
        ax_inset.clear() # Clear inset axes for each frame

    # Main CONUS plot
    merged_conus.plot(ax=ax_main, color=merged_conus['color'], edgecolor='white', linewidth=0.4)
    ax_main.axis("off")

    # Insets for non-CONUS states/territories
    for statefp, meta in inset_defs.items():
        ...
        if not subset.empty:
            ...
            # Place inset labels using transAxes for robust positioning
    ...
    # Set the title for this specific plot
    ax_main.set_title(title_suffix, fontsize=16)

    return ax_main.collections
...
def create_animation(event_data_list, policies_df, counties_shp, inset_defs, paths,
                      fps=0.25, output_filename="top_10_flood_events_animated.mp4"):
    """
    Creates an animation looping through the prepared event data.
    `event_data_list` is a list of dictionaries, each containing:
        'year', 'event_name', 'filtered_claims_df'
    """
    print(f"\nCreating animation: {output_filename}...")
    ...
    ax_insets = {}
    for statefp, meta in inset_defs.items():
        ax_insets[statefp] = fig.add_axes(meta['bounds'])

    _, initial_color_dict, _, _, _ = perform_bivariate_classification(
        aggregate_data(pd.DataFrame(columns=['countyCode', 'amountPaidOnBuildingClaim', 'amountPaidOnContentsClaim', 'amountPaidOnIncreasedCostOfComplianceClaim']), policies_df.head(1)))

    frames_data = []
    for event_dict in event_data_list:
        event_year = event_dict['year']
        event_name = event_dict['event_name']
        filtered_claims_df = event_dict['filtered_claims_df']

        if not filtered_claims_df.empty:
            agg_df = aggregate_data(filtered_claims_df, policies_df)
            if agg_df['avg_claim_amt'].sum() == 0 and agg_df['avg_coverage'].sum() == 0:
                 print(f"    No significant claim or policy data for {event_name} ({event_year}). Skipping this event in animation.")
                 continue

            agg_df, color_dict, darker_colors_matrix, claim_labels, policy_labels = perform_bivariate_classification(agg_df)
            merged_conus, merged_nonconus = merge_and_filter_counties(agg_df, counties_shp, color_dict)
            frames_data.append({
                'merged_conus': merged_conus,
                'merged_nonconus': merged_nonconus,
                'title': f"{event_name} ({event_year})",
                'color_dict_no_data_key': color_dict["NoData"],
                'legend_params': (darker_colors_matrix, claim_labels, policy_labels)
            })
        else:
            print(f"    No claims data found for {event_name} ({event_year}). Skipping this event in animation.")

    if not frames_data:
        print("No valid frames to animate. Animation skipped.")
        return

    create_bivariate_legend(fig, frames_data[0]['legend_params'][0],
                            frames_data[0]['legend_params'][1],
                            frames_data[0]['legend_params'][2])
    create_nodata_legend(fig, initial_color_dict)

    def update(frame_num):
        """
        This function is called for each frame of the animation.
        It updates the plot with data for the current frame.
        """
        frame_data = frames_data[frame_num]
        plot_single_frame(ax_main, ax_insets,
                          frame_data['merged_conus'],
                          frame_data['merged_nonconus'],
                          inset_defs,
                          frame_data['title'],
                          frame_data['color_dict_no_data_key'])
        return ax_main.collections + [ax_inset.collections for ax_inset in ax_insets.values()]


    plt.tight_layout() # Adjusted rect to pull content up a bit

    anim = FuncAnimation(fig, update, frames=len(frames_data), interval=1000/fps, blit=False, repeat=True)

    try:
        writer = animation.FFMpegWriter(fps=fps, metadata=dict(artist='NFIP Data Visualization'), bitrate=1800)
        anim.save(os.path.join(paths['outdir'], output_filename), writer=writer, dpi=300)
        print(f"Animation saved to: {os.path.join(paths['outdir'], output_filename)}")
    except ValueError as e:
        print(f"  Warning: FFMpegWriter failed ({e}). Trying PillowWriter for GIF.")
        try:
            gif_filename = output_filename.replace(".mp4", ".gif")
            writer = animation.PillowWriter(fps=fps)
            anim.save(os.path.join(paths['outdir'], gif_filename), writer=writer, dpi=300)
            print(f"  Animation saved as GIF to: {os.path.join(paths['outdir'], gif_filename)}")
        except Exception as e:
            print(f"  Error: PillowWriter also failed ({e}). Could not save animation.")

    plt.close(fig)


# --- Main Script for Animation ---

if __name__ == "__main__":
    print("Starting animation generation script...")

    paths = configure_paths()
    claims_raw_df, policies_df = load_data(paths['claims_fp'], paths['policies_fp'])

    # Identifying Top 10 Costliest Flood Events (duplicated for self-containment)
    print("\n--- Identifying Top 10 Costliest Flood Events for animation ---")
    con = duckdb.connect()
    ...
    try:
        ...
        print(f"Warning: CPI adjustment failed ({e}). Using raw 'paidTotalClaim' for ranking for animation data.")
        event_df = (
            base_event_df
            .assign(
    ...
    for index, row in event_df.iterrows():
        ...
        filtered_claims_df = filter_claims_for_plotting(claims_raw_df, [(event_year, event_name)])
        if not filtered_claims_df.empty:
            animation_data_list.append({
                'year': event_year,
                'event_name': event_name,
                'filtered_claims_df': filtered_claims_df
            })
    print(f"Prepared {len(animation_data_list)} events for animation.")

    create_animation(animation_data_list, policies_df, paths['counties_shp'], inset_defs, paths, fps=1)
